...
class RAGConfig:
    chunk_size: int = 500
    top_k_retrieval: int = 3 # Focused context for the agent
...
def analyze_csv_data(query: str) -> str:
    ...
    try:
        ...
        if match:
            ...
            target_column = None
            # Find a relevant column to search in
            for col in df.columns:
                if "rca" in col.lower() or "root cause" in col.lower() or "summary" in col.lower():
                    target_column = col
                    break
            if not target_column: return "Could not determine a relevant column to search for in the CSV."
...
if "messages" not in st.session_state:
    st.session_state.messages = [{"role": "assistant", "content": "Hi! How can I help you? You can ask me questions, upload an image via the sidebar, or trigger a real-time alert."}]
...
if "simulated_alert" in st.session_state and st.session_state.simulated_alert:
    alert = st.session_state.pop('simulated_alert')
    with st.chat_message("assistant"):
        with st.spinner("Agent is diagnosing the alert..."):
            if st.session_state.search_args:
                ...
                prompt_parts = [
                    "You are an AI diagnostic agent. You received a real-time alert and gathered context from multiple sources. Synthesize this information into a concise, actionable report. Provide a summary, a likely root cause, and a clear, numbered list of recommended actions.",
                    "---CONTEXT---", context]
                response_stream = generate_response_stream(prompt_parts)
                full_response = st.write_stream(response_stream)
                st.session_state.messages.append({"role": "assistant", "content": full_response})
            else:
                st.error("Cannot run diagnostic agent: Search index not available.")
...
if prompt or uploaded_image:
    ...
    with st.chat_message("user"):
        if uploaded_image:
            ...
            user_input_content.append(image)
        if prompt:
            st.markdown(prompt)
            user_input_content.append(prompt)
    
    # We add a placeholder to the messages list and update it later with the full response
    st.session_state.messages.append({"role": "user", "content": prompt or "Image uploaded"})
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            if any(kw in prompt_lower for kw in ["how many", "count", "total tickets"]):
                ...
                final_prompt_parts = ["Present the following result from the data analysis tool to the user:", context]
            else: # General RAG or Vision query
                context = ""
                if st.session_state.search_args and prompt:
                ...
                final_prompt_parts = [
                     "You are an expert AI support assistant. Answer the user's question based on the provided image (if any) and the text context. If no context is found, say so.",
                     f"\n---DOCUMENT CONTEXT---\n{context if context else 'No relevant documents found.'}",
                ]
                final_prompt_parts.extend(user_input_content)

            response_stream = generate_response_stream(final_prompt_parts)
            full_response = st.write_stream(response_stream)
            st.session_state.messages.append({"role": "assistant", "content": full_response})
