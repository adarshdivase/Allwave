...
class AVRecommendationEngine:
    ...
    def get_display_recommendations(self, room_length, room_width, capacity):
        ...
        # Calculate optimal display size using AVIXA standards
    ...
    def get_camera_recommendations(self, room_length, room_width, capacity):
        ...
        # Camera selection based on room size and capacity
    ...
    def get_control_recommendations(self, complexity_score):
        ...
        return [{
            ...
            'reason': f'Complexity score: {complexity_score}/5',
            'models': models,
    ...
    def _get_price_estimate(self, category, size_factor):
        ...
        price_ranges = {
            ...
            'camera': {
                range(1, 7): "$800 - $2,500",
                range(7, 13): "$2,500 - $8,000",
                range(13, 31): "$8,000 - $20,000"
            },
            'audio': {
                range(1, 7): "$1,500 - $4,000",
                range(7, 13): "$4,000 - $12,000", 
                range(13, 31): "$12,000 - $30,000"
            },
            'control': {
                range(1, 4): "$2,000 - $5,000",
                range(4, 6): "$5,000 - $15,000"
            }
        }
...
def create_3d_room_visualization(room_width, room_length, capacity, recommendations):
    ...
    # Room walls
    room_height = 3.0  # 3 meter ceiling
    
    # Floor
    fig.add_trace(go.Mesh3d(
        x=[0, room_width, room_width, 0],
        y=[0, 0, room_length, room_length],
        z=[0, 0, 0, 0],
        color='lightgray',
        opacity=0.3,
        name='Floor'
    ...
    # Walls
    wall_colors = ['lightblue', 'lightblue', 'lightblue', 'lightblue']
    walls = [
        # Front wall (where display is)
    ...
    display_height = display_width * 0.5625  # 16:9 aspect ratio
    display_x = (room_width - display_width) / 2
    display_y = room_length - 0.1
    display_z = 1.2  # Mounted at 1.2m height
    
    fig.add_trace(go.Mesh3d(
        x=[display_x, display_x + display_width, display_x + display_width, display_x],
        y=[display_y, display_y, display_y, display_y],
        z=[display_z, display_z, display_z + display_height, display_z + display_height],
        color='black',
        opacity=0.8,
        name='Display Screen'
    ))
    ...
    for i in range(chairs_per_side):
        ...
        chair_positions.append([table_x - 0.5, y_pos, 0])
        # Right side chairs  
        chair_positions.append([table_x + table_width + 0.5, y_pos, 0])
    
    for pos in chair_positions[:capacity]:
        fig.add_trace(go.Scatter3d(
            x=[pos[0]], y=[pos[1]], z=[pos[2] + 0.4],
            mode='markers',
    ...
    # Ceiling microphones (if recommended)
    if capacity > 6:
        mic_positions = [
            [room_width * 0.3, room_length * 0.4, room_height - 0.1],
            [room_width * 0.7, room_length * 0.4, room_height - 0.1],
            [room_width * 0.5, room_length * 0.7, room_height - 0.1]
        ]
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            aspectmode='manual',
            aspectratio=dict(x=1, y=room_length/room_width, z=0.5)
        ),
        showlegend=False,
        height=600
    )
...
with st.sidebar:
    ...
    room_type = st.selectbox(
        "Room Type",
        ["Small Meeting Room", "Medium Conference Room", "Large Boardroom", "Training Room", "Auditorium"]
    )
    ...
    with st.expander("Advanced Options"):
        ...
        budget_range = st.selectbox(
            "Budget Range",
            ["Economy ($10K-25K)", "Standard ($25K-50K)", "Premium ($50K-100K)", "Enterprise ($100K+)"]
        )
        special_requirements = st.multiselect(
            "Special Requirements",
            ["Recording Capability", "Live Streaming", "Wireless Presentation", "Room Booking Display", "Environmental Controls"]
        )
...
with tab1:
    ...
    with col1:
        # Generate recommendations for 3D viz
...
with tab3:
    ...
    st.download_button(
        label="📥 Download Equipment Specifications",
        data=csv_data,
        file_name=f"room_config_{room_type.replace(' ', '_')}_{capacity}pax.csv",
        mime="text/csv"
    )
...
with tab4:
    ...
    requirements = [
        "🔌 Power outlets: Minimum 6 dedicated 15A circuits",
        "🌐 Network infrastructure: CAT6A cabling to all device locations", 
        "📺 Display mounting: Reinforced wall mount or ceiling suspension",
        "🎤 Audio infrastructure: Conduit for microphone cabling",
        "❄️ Environmental: HVAC considerations for equipment cooling",
        "🔒 Security: Equipment lockdown and access control"
    ]
